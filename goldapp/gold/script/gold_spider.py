...
class gold_data:
    ...
    def __init__(self):
        self.save_path = os.path.dirname(os.path.dirname(__file__)) + '/input/'
        self.log_path = os.path.dirname(os.path.dirname(__file__)) + '/log/'
        ...
        # self.log = log_function.log_uitil().logging_function()
    ...
    def get_data_by_page_number(self, page_number):
        ...
        for tar_url, num in zip(target_url, update_version_date):
            try:
                ...
                # logging.error(f">>> [ERROR] url error..{tar_url}")
    ...
    def get_max_page_number(self):
        try:
            ...
            # logging.info(f'max_page_number:{max_page_number}')
            return max_page_number
        except Exception as e:
            print(">>>> [ERROR] get max page number error..")
            logging.error(">>>> [ERROR] get max page number error..")
...
if __name__ == '__main__':
    ...
    # error_data.noraml_batch_run()
    # error_data.tread_pool_batch_run()
    end_time = time.time()
    print("cost:", round((end_time - start_time), 2))
    # logging.info(f"cost:{round((end_time - start_time), 2)}")
